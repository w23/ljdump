...
def flatresponse(response):
    r = {}
    while True:
        name = response.readline().decode('utf-8')
        if len(name) == 0:
            break
        if name[-1] == '\n':
            name = name[:len(name)-1]
        value = response.readline().decode('utf-8')
        if value[-1] == '\n':
...
def getljsession(server, username, password):
    r = urllib.request.urlopen(server+"/interface/flat", "mode=getchallenge".encode('utf-8'))
    response = flatresponse(r)
    r.close()
    r = urllib.request.urlopen(server+"/interface/flat", ("mode=sessiongenerate&user=%s&auth_method=challenge&auth_challenge=%s&auth_response=%s" % (username, response['challenge'], calcchallenge(response['challenge'], password))).encode('utf-8'))
    response = flatresponse(r)
    r.close()
